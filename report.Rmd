...
Figure \@ref(fig:trip-times) shows that travel times southbound away from the city are larger and more dispersed than the travel times northbound towards the city.
...
The distribution of the motor vehicle travel times per direction appears as follows:

```{r total-times, fig.cap="Both direction have similar travel time profiles"}
ggplot(links_times, aes(links_time, fill = direction)) +
  geom_density(alpha = 0.4) +
  scale_x_continuous(labels = label_comma()) +
  scale_y_continuous(labels = label_comma()) +
  labs(x = "Vehicle Travel Time (Seconds)",
       fill = "Direction",
       title = "Vechile Travel Time Distributions") +
  theme_bw() +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank())
...
Figure \@ref(fig:total-times) shows the travel times for both directions are very similar.

With all the data now complete, the analysis between the two travel times is ready to be performed.

# Analysis

The goal of the analysis to ascertain the extent of the relationship between the variations in the motor vehicle travel times and the variations in the bus trip travel times, the variation is in reference to travel times during the same time frame across the entire period. In other words, is the variation from the normal travel time by vehicles matched by a variation from the normal travel time by buses? If so, by what extent?
...
## Initial Travel Times Comparison
...
```{r bus-vehicles-time, fig.cap="Vehicles are faster in both directions"}
...
Figure \@ref(fig:bus-vehicles-time) shows that northbound traffic is generally faster than southbound traffic, buses are slower than vehicles, and the shapes of the distributions between both types are very similar.
