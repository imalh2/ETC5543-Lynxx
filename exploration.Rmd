...
stops_delays <- combined %>%
  ...
  filter(!(stop_direction == "3387-1" & delay_mins > 60)) %>% 
  group_by(stop_direction) %>% 
  summarise(avg_delay = mean(delay_mins),
            occurrences = n()) %>%
  ungroup() %>%
...
stops_combined <- combined %>% 
  semi_join(top_stops_delays, by = c("stop_direction"))

stops_combined %>% 
  ggplot(aes(x = timestamp, y = delay_mins, color = stop_direction)) +
  geom_point() +
  geom_line()
```

## Delays per Consecutive Stops per Direction

```{r}
consec_stops_delays <- combined %>%
  group_by(stop_pair_dir) %>% 
  summarise(avg_delay = mean(delay_mins),
            occurrences = n()) %>%
  ungroup() %>%
  filter(occurrences > mean(occurrences)) %>%
  arrange(-avg_delay)
```
...
routes_delays <- combined %>%
  filter(!(route_direction %in% c("224M-0", "G20-0", "750B-1", "157-0", "117-0") & delay_mins > 40)) %>% 
  group_by(route_direction) %>%
  summarise(avg_delay = mean(delay_mins),
...
routes_combined <- combined %>% 
  semi_join(top_routes_delays, by = c("route_direction"))

ggplot(routes_combined, aes(x = timestamp, y = delay_mins, color = route_direction)) +
  geom_point() +
  geom_line()
```
