...
# Keep sites within 300m of any stop
...
selected_routes %>% 
  leaflet() %>% 
  addTiles() %>% 
  addCircles(lng = ~ stop_lon, lat = ~ stop_lat, color = ~ route_color, popup = ~ stop_id) %>% 
  addCircles(data = sites_map, lng = ~ longitude, lat = ~ latitude, popup = ~ as.character(id), color = "green")
```
