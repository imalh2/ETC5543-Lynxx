...
southrd_link_stats_raw <- read_csv(here("data/South-Rd/link_5minstats_Mar.csv"))
```
...
# Direction 1
travel_times_dir1 <- travel_times_southrd %>% 
  filter(direction == directions[1])
...
# Direction 2
travel_times_dir2 <- travel_times_southrd %>% 
  filter(direction == directions[2])
...
```{r}
paste("Correlation:", round(with(travel_times_southrd, cor(bus_time, links_time)), 2))
...
### Hourly pattern of links time and bus trips time
...
```{r}
paste("All times correlation:", round(with(travel_times_southrd_std, cor(bus_time_std, links_time_std)), 2))

paste("Peak times correlation:", round(with(filter(travel_times_southrd_std, rush != "neither"), 
...
travel_times_std_cor <- travel_times_southrd_std %>%
  filter(rush != "neither") %>%
...
# Hourly correlation
travel_times_std_hr_cor <- travel_times_southrd_std %>%
  filter(rush != "neither") %>%
...
```{r}
# Try all variables
all_lm <- lm(bus_time_std ~ links_time_std + rush*direction, travel_times_southrd_std)
summary(all_lm)
drop1(all_lm)
step(all_lm, trace = FALSE)

# Only links_time_std is needed
all_lm <- lm(bus_time_std ~ links_time_std, travel_times_southrd_std)
summary(all_lm)
tidy(all_lm)
glance(all_lm)
...
```{r data}
## List of links on segment and their geometries
marionrd_links_raw <- read_csv(here("data/Mariod-Rd/Marion Rd Links.csv"))

## Routes, trips, and stops on segment
marionrd_routes_stops_raw <- read_csv(here("data/Mariod-Rd/Marion Rd Routes Stops.csv"))

## GTFS-R Trip Updates
marionrd_updates_raw <- read_csv(here("data/Mariod-Rd/trip_updates_MarionRd_Mar.csv"))

## Link 5 minute statistics
marionrd_link_stats_raw <- read_csv(here("data/Mariod-Rd/link_5minstats_Mar.csv"))
```
...
travel_times_dir1 <- travel_times_marionrd %>% 
  filter(direction == directions[1])
...
travel_times_dir2 <- travel_times_marionrd %>% 
  filter(direction == directions[2])
...
```{r}
paste("Correlation:", round(with(travel_times_marionrd, cor(bus_time, links_time)), 2))
...
```{r}
paste("All times correlation:", round(with(travel_times_marionrd_std, cor(bus_time_std, links_time_std)), 2))

paste("Peak times correlation:", round(with(filter(travel_times_marionrd_std, rush != "neither"), 
                                            cor(bus_time_std, links_time_std)), 2))
...
travel_times_std_cor <- travel_times_marionrd_std %>%
  filter(rush != "neither") %>%
  group_by(time, rush, direction) %>% 
  summarise(
    correlation = cor(bus_time_std, links_time_std),
...
travel_times_std_hr_cor <- travel_times_marionrd_std %>%
  filter(rush != "neither") %>%
  group_by(hour, rush, direction) %>% 
  summarise(
    correlation = cor(bus_time_std, links_time_std),
...
```{r}
all_lm <- lm(bus_time_std ~ links_time_std + rush*direction, travel_times_marionrd_std)
summary(all_lm)
drop1(all_lm)
step(all_lm, trace = FALSE)

all_lm <- lm(bus_time_std ~ links_time_std, travel_times_marionrd_std)
summary(all_lm)
tidy(all_lm)
glance(all_lm)
...
# South Rd vs Marion Rd

```{r}
travel_times_both <- trip_times_agg_southrd %>% 
  inner_join(trip_times_agg_marionrd,
             by = c("day", "time", "hour", "rush", "direction"),
             suffix = c("_southrd", "_marionrd")) %>% 
  select(-c(number_buses_southrd, number_buses_marionrd))

travel_times_both_piv <- travel_times_both %>% 
  pivot_longer(cols = bus_time_southrd:bus_time_marionrd, names_to = "road", values_to = "travel_time")

travel_times_both_std <- trip_times_southrd_std %>%
  inner_join(trip_times_marionrd_std,
               by = c("day", "time", "hour", "rush", "direction"),
               suffix = c("_southrd", "_marionrd")) %>% 
  select(-c(number_buses_southrd, number_buses_marionrd, period_buses_no_southrd, period_buses_no_marionrd)) %>% 
  mutate(std_travel_diff = bus_time_std_southrd - bus_time_std_marionrd)

travel_times_both_std_piv <- travel_times_both_std %>% 
    pivot_longer(cols = bus_time_std_southrd:bus_time_std_marionrd, names_to = "road", values_to = "std_travel_time")

both_trips_30min <- trip_times_30min_southrd %>% 
  inner_join(trip_times_30min_marionrd,
             by = c("time", "direction", "rush"),
             suffix = c("_southrd", "_marionrd")) %>% 
  pivot_longer(cols = bus_time_southrd:bus_time_marionrd, names_to = "road", values_to = "value")
```

## Travel times comparison

```{r}
travel_times_dir1 <- travel_times_both %>% 
  filter(direction == directions[1])

travel_times_piv_dir1 <- travel_times_both_piv %>% 
  filter(direction == directions[1])

travel_times_piv_dir1 %>%
  ggplot(aes(time, travel_time, color = road)) +
  geom_line() +
  facet_wrap(vars(day)) +
  ggtitle(directions[1]) +
  theme_bw() +
...
travel_times_dir2 <- travel_times_both %>% 
  filter(direction == directions[2])

travel_times_piv_dir2 <- travel_times_both_piv %>% 
  filter(direction == directions[2])
...
travel_times_both %>% 
  ggplot(aes(bus_time_southrd, bus_time_marionrd)) +
  geom_point(alpha = 0.3) +
  geom_smooth(se = FALSE) +
  theme_bw()
...
### Correlation between travel times

```{r}
paste("Correlation:", round(with(travel_times_both, cor(bus_time_southrd, bus_time_marionrd)), 2))

travel_times_both %>% 
  group_by(direction) %>% 
  summarise(correlation = round(cor(bus_time_southrd, bus_time_marionrd), 2))
```

```{r}
ggplot(both_trips_30min, aes(time, value)) +
  geom_line(alpha = 0.2) +
  geom_line(data = filter(both_trips_30min, rush == "morning"), color = "blue", size = 0.5) +
  geom_line(data = filter(both_trips_30min, rush == "evening"), color = "blue", size = 0.5) +
  facet_grid(rows = vars(road), cols = vars(direction), scales = "free_y") +
  annotate("rect", xmin = as_hms("06:00:00"), xmax = as_hms("10:00:00"), ymin = -Inf, ymax = Inf,
           fill = "red", alpha = .1) +
  annotate("rect", xmin = as_hms("15:00:00"), xmax = as_hms("19:00:00"), ymin = -Inf, ymax = Inf,
           fill = "red", alpha = .1) +
...
### Correlations between standardised travel times

```{r}
# Morning Northbound
travel_times_std_NB_morn <- travel_times_both_std %>% 
  filter(direction == "NB",
         rush == "morning")

travel_times_std_piv_NB_morn <- travel_times_both_std_piv %>% 
  filter(direction == "NB",
         rush == "morning")

travel_times_std_piv_NB_morn %>%
  ggplot(aes(time, std_travel_time, color = road)) +
  geom_line() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylim(c(-3,3)) +
  facet_wrap(vars(day)) +
...
# Evening Southbound
travel_times_std_SB_eve <- travel_times_both_std %>% 
  filter(direction == "SB",
         rush == "evening")

travel_times_std_piv_SB_eve <- travel_times_both_std_piv %>% 
  filter(direction == "SB",
         rush == "evening")
...
```{r}
travel_times_both_std %>% 
  ggplot(aes(bus_time_std_southrd, bus_time_std_marionrd)) +
  geom_point(alpha = 0.3) +
  geom_smooth(se = FALSE) +
  ggtitle("All Times") +
  theme_bw()
...
```{r}
paste("All times correlation:", round(with(travel_times_both_std, cor(bus_time_std_southrd, bus_time_std_marionrd)), 2))

paste("Peak times correlation:", round(with(filter(travel_times_both_std, rush != "neither"), 
                                            cor(bus_time_std_southrd, bus_time_std_marionrd)), 2))

# Peak times and direction correlation
travel_times_both_std %>% 
  filter(rush != "neither") %>% 
  group_by(rush, direction) %>% 
  summarise(correlation = round(cor(bus_time_std_southrd, bus_time_std_marionrd), 2)) %>% 
  ungroup()

# Time period correlation
travel_times_std_cor <- travel_times_both_std %>%
  filter(rush != "neither") %>%
  group_by(time, rush, direction) %>% 
  summarise(
    correlation = cor(bus_time_std_southrd, bus_time_std_marionrd),
    sample_size = n()) %>% 
  ungroup() %>% 
  na.omit()
...
# Hourly correlation
travel_times_std_hr_cor <- travel_times_both_std %>%
  filter(rush != "neither") %>%
  group_by(hour, rush, direction) %>% 
  summarise(
    correlation = cor(bus_time_std_southrd, bus_time_std_marionrd),
