...
```{r}
# Hours with no updates
sort(unique(hour(updates$timestamp)))

# Hours with no updates after filtering to stops
sort(unique(hour(updates_stops_filtered$timestamp)))

missing_times <- updates %>% 
  distinct(as_hms(ceiling_date(timestamp, "1 minutes")))
```
