...
# Joining
## Join routes_trips_stops with stops to get stopd coordinates
routes_trips_stops <- routes_trips_stops %>% 
...
routes_trips_stops %>% 
    leaflet() %>% 
    addTiles() %>% 
    addCircles(lat = ~stop_lat, lng = ~stop_lon, label = ~ stop_id, fillOpacity = 1)
```
