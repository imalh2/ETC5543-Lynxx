...
## Average delay per stop per time (exclude weekends)
...
```{r}
# # Join delay, sites congestion and links congestion
# delay_congestion_stamp <- delay_stamp %>% 
#   full_join(sites_cong_logtime, by = c("timestamp" = "logtime")) %>% 
#   full_join(links_cong_logtime, by = c("timestamp" = "logtime"), suffix = c("_sites", "_links")) %>% 
#   pivot_longer(cols = !timestamp, names_to = "stat", values_to = "value")
# 
# delay_congestion_stamp %>% 
#   #filter(stat != "avg_duration") %>% 
...
```{r}
delay_congestion_time <- delay_time %>% 
  full_join(sites_cong_time) %>% 
  full_join(links_cong_time, by = "time", suffix = c("_sites", "_links")) %>% 
  pivot_longer(cols = !time, names_to = "stat", values_to = "value")

delay_congestion_time %>% 
  #filter(stat != "avg_duration") %>% 
