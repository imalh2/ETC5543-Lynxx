...
updates <- updates_raw %>% 
  ...
  mutate(start_date = ymd(start_date),
         # Add 10.5 hours to UTC to match Adelaide time
...
links_order_up <- links_order_up[!links_order_up %in% c(1603, 1604, 1606, 3573, 3574)]
...
# Missing link between Ayliffs Road and Celtic Avenue. Locate a link between the two
link_ayliffs_celt_up <- links_south_rd %>%
  filter(str_detect(tolower(name), "ayliffes"),
...
for (link in unique(links_map_up$linkid)) {
  ...
  map_up <- map_up %>% 
    addPolylines(data = map_df, lng = ~longitude, lat = ~latitude, color = "green", opacity = 1, label = ~linkid)
}
...
delay_cong_sites_rush <- delay_cong_sites %>% 
