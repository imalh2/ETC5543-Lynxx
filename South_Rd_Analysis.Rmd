...
dir1_start_loc <- "tonsley"
dir1_end_loc <- "anzac"

# Retrieve sequence of links composing segment
linkids_dir1 <- linkids_dir1[min(which(str_starts(tolower(linkids_dir1$start_loc), tolower(dir1_start_loc)))):
                               max(which(str_starts(tolower(linkids_dir1$end_loc), tolower(dir1_end_loc)))),] %>% 
  group_by(start_loc) %>%
...
first_word_pattern <- "(?:^|(?:[.!?]\\s))(\\w+)"

linkids_dir2 <- linkids_dir2[which(str_starts(tolower(linkids_dir2$start_loc), tolower(dir2_start_loc))):
                               which(str_starts(tolower(linkids_dir2$end_loc), tolower(dir2_end_loc))),] %>% 
  # Remove links where end_loc does not have a matching start_loc in another link
  filter(str_extract(end_loc, first_word_pattern) %in% c(str_extract(start_loc, 
                                                                     first_word_pattern),
                                                         str_extract(end_loc[length(end_loc)], 
                                                                     first_word_pattern))) %>% 
  filter(str_extract(end_loc, first_word_pattern) %in% c(str_extract(start_loc, 
                                                                     first_word_pattern),
                                                         str_extract(end_loc[length(end_loc)], 
  ...
  filter(length == min(length)) %>%
  ungroup() %>% 
  filter(str_extract(start_loc, first_word_pattern) %in% c(str_extract(start_loc[1], 
                                                                       first_word_pattern), 
                                                           str_extract(end_loc, 
                                                                       first_word_pattern))) %>% 
  filter(str_extract(start_loc, first_word_pattern) %in% c(str_extract(start_loc[1], 
