...
```{r}
# Ceiling date by aggregate duration
delays_agg <- updates_stops_filtered  %>% 
  mutate(timestamp = ceiling_date(timestamp, "10 mins")) %>% 
  mutate(time = str_sub(timestamp, start = 12), .after = timestamp)

# Without direction

## Average delay per stop per timestamp
stop_delays_stamp <- delays_agg %>% 
...
stop_delays_stamp_dir <- delays_agg %>% 
...
sites_agg <- site_stats  %>% 
  ...
  mutate(time = str_sub(logtime, start = 12), .after = logtime)
