...
-   Adelaide's population increased from 1.1 to 1.3 million between 2006 and 2016, along with 66 million additional travel km's

-   Costly road congestion projected to increase in the coming years. \$1.4 billion in 2016 to \$2.6 billion in 2031

-   Client is South Australia Department of Infrastructure and Transport (DIT)

-   Client has a large collection of mostly unused traffic data
...
1.  Investigate the extent of the relationship between the **variation** in bus travel times vs the **variation** in motor vehicle travel time in a given segment of road. Provides an indication of road robustness to congestion
...
::: notes
-   GTFS developed by Google. Open source. Worldwide
...
Segment route ID's extracted through Tableau. Needed for trip updates data
...
stops_segment %>% 
  ...
  mutate(Description = c("Unique stop identifier",
                         ...
                         "Road direction of the stop. Created")) %>% 
  kable() %>% 
  kable_styling()
```
:::
...
::: notes
-   Unedited data according to the routes identified

-   Original method used delays. Changed to time to remove the possibility that we are measuring how accurately the schedule predicts and/or buffers for congestion.
:::
...
Different pairs of start-end stops per direction lead to inconsistent travel times due to different distances
...
::: notes
-   Explain how trip time is calculated (arrival time of last stop - first stop)

-   Different travel times are incomparable and can lead to issues in calculations later
:::
...
links_segment %>%
  ...
  mutate(Description = c("Unique link identifier",
                         ...
                         "Latitude",
                         "Longitude",
                         "Name of link start location. Created",
                         "Name of link end location. Created",
                         "Latitude of the link's first shape coordinate. Created",
                         "Latitude of the link's last shape coordinate. Created",
...
-   5 minute aggregated statistics per link
...
-   Retrieve statistics of identified links from AWS
...
link_stats %>%
  ...
  mutate(Description = c("Current interval timestamp",
                         ...
                         "The link direction of travel. From links dataset",
                         "The link length in metres. From links dataset",
                         "Calculated speed in km/h. Created"
                         )) %>% 
  kable() %>% 
  kable_styling()
```
...
::: notes
-   Link statistics for links on segment as well as links used for imputation
:::
...
::: notes
-   30 minute aggregates across all days

-   morning and evening rush hours highlighted

-   northbound travel towards the city, both rush hour times display a similar level of travel time for both types, and the travel time in the rush hours are not much greater than non-rush hour times. This is an unexpected result as it is expected that travel times northbound towards the city would be higher in the morning rush hour

-   Southbound travel away from the city, however, follows expectations as the travel time for both types dramatically increases in the evening rush hour as workers leave the city.
:::
...
::: notes
positive relationship, especially southbound evening
:::

## Standardize Travel Times

-   Allows cross examination of variations from "normal"

-   Function `standardizer` created to standardize both types of travel times with respect to one of:

    -   five minute period across the month

    -   hour across the month

    -   rush hour across the month

Function default is hour
...
::: notes
-   Travel time naturally changes across the day

-   Need to compare to "normal" per timeframe

-   if the vehicle travel time varies by a certain level relative to the usual travel time during the same time frame, can we observe a reflection of this variation in the bus travel time? If so, by how much?

-   if correlation is low, bus network more resilient

-   As the time frame widens, more data is available for standardization, but the standardization takes a wider time range, leading to bias

-   Remove observations more than 3 standard deviations away

-   Maximum 21 5 minute periods
:::
