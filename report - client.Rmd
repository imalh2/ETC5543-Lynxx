...
ggplot(trip_time_delay, aes(delay_diff/60, color = direction)) +
  geom_boxplot() +
...
Table \@ref(tab:general-corr) shows that the travel times between buses and vehicles are high correlated in the morning northbound towards the city, and in the evening southbound away from the city.
...
Figure \@ref(fig:nb-std-calendar) and figure \@ref(fig:sb-std-calendar) show that variations in vehicle travel times are in fact closely matched by variations in bus travel times, especially during the morning and northbound towards the city, where the variations also exist in greater magnitudes.
...
```{r hourly-corr, fig.cap="High correlation is more uniform in the evening southbound"}
# Hourly correlation
...
travel_times_std_hr_cor %>% 
  ...
  labs(x = "Hour",
       y = "Correlation",
       title = "Correlation per Hour per Direction") +
  theme_bw()
```

Figure \@ref(fig:hourly-corr) shows that high correlation exists in the morning northbound towards the city between 7am and 9am, while in the evening southbound away from the city, high correlation exists throughout the rush hour and peaking betwen 6pm and 7pm.
