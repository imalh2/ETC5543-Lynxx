...
travel_times %>% 
  ggplot(aes(links_time, bus_time)) +
...
#### Standardized based on 5 minute time periods
...
trip_times_std <- trip_times_5min %>% 
  ...
  ungroup() %>% 
  select(start_date, time, hour, rush, direction, bus_time_std) %>% 
...
#### Standardized based on hour

```{r}
# Trip time standardized per hour per direction
trip_times_hr_std <- trip_times_5min %>% 
...
# Calculate travel time across all links standardized per hour per direction
links_times_hr_std <- links_times %>% 
  group_by(hour, direction) %>% 
  mutate(links_time_std = as.vector(scale(links_time))) %>% 
  ungroup() %>% 
  select(-links_time, -logtime)
...
# Combine
travel_times_hr_std <- trip_times_hr_std %>%
  inner_join(links_times_hr_std, by = c("start_date" = "day", "time", "hour", "rush", "direction")) %>% 
  rename(day = start_date)

travel_times_hr_std_piv <- travel_times_hr_std %>% 
  pivot_longer(cols = bus_time_std:links_time_std, names_to = "type", values_to = "travel_time")
```
...
```{r}
# All times correlation
with(travel_times_hr_std, cor(bus_time_std, links_time_std))

# Peak times correlation
with(filter(travel_times_hr_std, rush != "neither"), cor(bus_time_std, links_time_std))
...
# Time period correlation
travel_times_hr_std_cor <- travel_times_std %>%
  filter(rush != "neither") %>%
  group_by(time, rush, direction) %>% 
  summarise(correlation = cor(bus_time_std, links_time_std)) %>% 
  ungroup() %>% 
...
# Hourly correlation
travel_times_hr_std_hr_cor <- travel_times_std %>%
  filter(rush != "neither") %>%
  group_by(hour, rush, direction) %>% 
  summarise(correlation = cor(bus_time_std, links_time_std)) %>% 
  ungroup() %>% 
...
#### Standardize based on peak time periods

```{r}
# Trip time standardized per peak time per direction
trip_times_peak_std <- trip_times_5min %>% 
  group_by(rush, direction) %>% 
  mutate(bus_time_std = as.vector(scale(trip_time))) %>% 
  ungroup() %>% 
  select(start_date, time, hour, rush, direction, bus_time_std) %>% 
...
# Calculate travel time across all links standardized per peak per direction
links_times_peak_std <- links_times %>% 
  group_by(rush, direction) %>% 
  mutate(links_time_std = as.vector(scale(links_time))) %>% 
  ungroup() %>% 
  select(-links_time, -logtime)
...
travel_times_peak_std <- trip_times_peak_std %>%
  inner_join(links_times_peak_std, by = c("start_date" = "day", "time", "hour", "rush", "direction")) %>% 
  rename(day = start_date)
...
# Peak times correlation
with(filter(travel_times_peak_std, rush != "neither"), cor(bus_time_std, links_time_std))
...
# Time period correlation
travel_times_peak_std_cor <- travel_times_std %>%
  filter(rush != "neither") %>%
  group_by(time, rush, direction) %>% 
  summarise(correlation = cor(bus_time_std, links_time_std)) %>% 
  ungroup() %>% 
...
# Hourly correlation
travel_times_peak_std_hr_cor <- travel_times_std %>%
  filter(rush != "neither") %>%
  group_by(hour, rush, direction) %>% 
  summarise(correlation = cor(bus_time_std, links_time_std)) %>% 
  ungroup() %>% 
