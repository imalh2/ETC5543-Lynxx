...
```{r}
stops_needed <- clean_names(routes_trips_stops_raw) %>% 
...
stops_segment <- full_stops_raw %>% 
  ...
  filter(stop_id %in% stops_needed) %>% 
...
stops_to_city <- stops_segment %>% 
  filter(direction == "to_city")

stops_from_city <- stops_segment %>% 
...
# Take top 4 occurring stops pair for each direction
...
updates_segment <- updates %>% 
  ...
  semi_join(top_first_last_stops, by = c("direction_id", "first_stop", "last_stop")) %>% 
  group_by(start_date, trip_id) %>% 
  arrange(stop_sequence, .by_group = TRUE) %>%
  # Remove any observations that have an arrival_time later than any following arrival_time in the trip
  # AND the timestamp is earlier than any following timestamps in the trip.
  ...
  # Remove if the arrival_time is less than the previous arrival_time AND the timestamp is older
  # than the previous timestamp
  filter(!(arrival_time < lag(arrival_time, order_by = stop_sequence, default = ymd("0000-01-01", tz = "UTC")) &
             timestamp < lag(timestamp, order_by = stop_sequence, default = ymd("0000-01-01", tz = "UTC")))) %>%
  # If after the above two consecutive stops have the same arrival_time, remove the one
  # with an older timestamp
  filter(!(arrival_time == lead(arrival_time, order_by = stop_sequence, default = ymd("9999-01-01", tz = "UTC")) &
           timestamp < lead(timestamp, order_by = stop_sequence, default = ymd("9999-01-01", tz = "UTC")))) %>%
  # Remove updates with arrival_time greater than next arrival_time due to measurement error
  # (if 2 rows are wrong one after another, only one is removed)
  filter(!(arrival_time > lead(arrival_time, default = ymd("9999-01-01", tz = "UTC")) &
             lead(arrival_time, default = ymd("9999-01-01", tz = "UTC")) > lag(arrival_time, default = ymd("0000-01-01", tz = "UTC")))) %>%
  mutate(#prior_arrival = lag(arrival_time),
         #post_arrival = lead(arrival_time),
         to_stop_time = as.numeric(arrival_time - lag(arrival_time))) %>% 
  ...
  # Remove remaining negative to_stop_times
  filter(to_stop_time >= 0) %>% 
  select(-first_stop, -last_stop)
  # group_by(start_date, trip_id) %>%
  # mutate(trip_time = as.numeric((last(arrival_time, order_by = stop_sequence) -
  #                                  first(arrival_time, order_by = stop_sequence)), units = "secs")) %>%
  # ungroup() %>%
...
```{r}
glimpse(updates_segment)
summary(updates_segment)

# Check if a trip_id happens more than once per day. Included stop_id because
# trip_id will be repeated for each stop in the trip

ggplot(updates_segment, aes(to_stop_time)) +
  geom_histogram()
...
# Check if large positive or negative to_stop_times are related to prior_stop_time
ggplot(updates_segment,aes(to_stop_time, delay)) +
  geom_point() +
  theme_minimal()

ggplot(updates_segment,aes(to_stop_time, post_stop_time)) +
  geom_point() +
  theme_minimal()
```

If stop_time is large and post_stop_time is large negative, then definitely error in data entry of one stop. If stop_time is large, and post_stop_time is positive, then probably valid; if large delay is involved for the first stop, then makes sense that the next stops also have large delays.

stop_time is negative due to arrival_time going back starting from a certain stop and forward. Likely due to update happening way later for the previous stops. Or because maybe bus arrives earlier than predicted and the following bus stop is predicted to arrive even earlier than originally thought, resulting in a negative to_stop_time
