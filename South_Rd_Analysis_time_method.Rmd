...
updates <- updates_raw %>%
  ...
  mutate(to_stop_time = as.numeric(arrival_time - lag(arrival_time, order_by = stop_sequence))) %>% 
  ungroup() %>% 
...
sequence_times_mis <- updates %>% 
  group_by(route_id, direction_id, start_date, trip_id) %>%
  mutate(chrono_error = lag(arrival_time) > arrival_time) %>%
  filter(any(chrono_error == TRUE)) %>% 
  ungroup() %>% 
  mutate(chrono_error = replace_na(chrono_error, 0)) %>%
  arrange(start_date, trip_id, stop_sequence)

sequence_times_mis %>% count(chrono_error)
