...
link_5min_stats_raw <- read_csv(here("data/South-Rd/link_5minstats_Mar.csv"))
...
# Check against 5-minute stats
link_5min_stats <- link_5min_stats_raw %>% 
...
link_stats_agg <- link_stats %>% 
  mutate(logtime = ceiling_date(logtime, "5 mins")) %>%
  group_by(logtime, linkid) %>% 
  summarise(tt = mean(tt)) %>% 
  ungroup()

# Different
link_check <- link_stats_agg %>% 
  left_join(link_5min_stats, by = c("logtime", "linkid"))
...
links_to_city <- links_segment %>%
  filter(direction == "to_city") %>% 
  # Build map from lower end of map to upper
  arrange(start_lat, linkid, ordernumber) %>%
  distinct(linkid, .keep_all = TRUE) %>% 
  # Start and end at same locations as bus stops, in this case Walsh Ave and Anzac Hwy
  filter(cumany(tolower(start_loc) == "walsh ave"),
         cumall(!(tolower(lag(end_loc, default = "zzz")) == "anzac hwy")),
         # Remove links where end_loc does not have a matching start_loc in another link
         end_loc %in% c(start_loc, end_loc[length(end_loc)])) %>%
...
links_to_city_map <- links_segment %>% 
  filter(linkid %in% links_to_city) %>% 
  arrange(start_lat, ordernumber)
...
links_from_city <- links_segment %>%
  ...
  arrange(-start_lat, linkid, ordernumber) %>%
  distinct(linkid, .keep_all = TRUE) %>% 
  filter(cumany(tolower(start_loc) == "anzac hwy"),
         cumall(!(tolower(lag(end_loc, default = "zzz")) == "walsh ave")),
         end_loc %in% c(start_loc, end_loc[length(end_loc)])) %>% 
  # If more than one link has the same start_loc, pick the shorter one
  group_by(start_loc) %>% 
  filter(length == min(length)) %>% 
...
links_from_city_map <- links_segment %>% 
  filter(linkid %in% links_from_city) %>% 
  arrange(-start_lat, ordernumber)
...
# Save linkid's both directions
links_both <- c(links_to_city, links_from_city)
```
...
```{r}
compare_trips_sites(timeframe = "15 minutes")
```
