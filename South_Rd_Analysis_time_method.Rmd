...
updates_stop_times <- updates %>% 
  ...
  filter(!(delay - lag(delay, order_by = stop_sequence, default = 0) > 1000 & delay - lead(delay, order_by = stop_sequence, default = 0) > 1000)) %>%
  ...
  # Remove if the arrival_time is less than the previous arrival_times AND the timestamp is older
  # than the previous timestamps
  ...
  # If arrival_time of a stop is less than prior stops but they all have the same timestamp,
  # it's not possible to know which is correct. Assume earlier stop_sequence is correct
  # since it is closer when the update is made
  ...
  # If two stops have the same arrival_time and same time_stamp, remove the one with a higher stop sequence.
  # Make sure only 2 stops and not more
  filter(!(arrival_time != lead(arrival_time, order_by = stop_sequence, default = ymd("9999-01-01", tz = "UTC")) &
             arrival_time == lag(arrival_time, order_by = stop_sequence, default = ymd("0000-01-01", tz = "UTC")) &
  ...
  # Remove trips whith multiple repeating arrival_times
...
updates_stop_times <- updates_stop_times %>% 
   semi_join(top_first_last_stops2, by = c("direction_id", "first_stop", "last_stop"))
...
updates_segment_trim_start <- updates %>% 
  # Keep updates for trips that have first and last stop from pairs identified
  semi_join(top_first_last_stops, by = c("direction_id", "first_stop", "last_stop")) %>%
  # Remove delays over 3000 because they are most likely errors
  filter(delay < 3000,
         delay > -350) %>% 
  group_by(start_date, trip_id) %>%
  arrange(stop_sequence, .by_group = TRUE) %>%
...
updates_segment_trim_end <- updates %>% 
  # Keep updates for trips that have first and last stop from pairs identified
  semi_join(top_first_last_stops, by = c("direction_id", "first_stop", "last_stop")) %>%
  # Remove delays over 3000 because they are most likely errors
  filter(delay < 3000) %>% 
...
test <- updates_segment %>% 
  filter(start_date == "2022-03-15", trip_id == 643511)

time_delay <- updates_trip_time %>% 
  group_by(start_date, trip_id) %>% 
  filter(abs(delay) == max(abs(delay))) %>%
  distinct(start_date, trip_id, .keep_all = TRUE) %>% 
  ungroup()

time_delay_2000 <- updates_segment_2000 %>% 
  group_by(start_date, trip_id) %>% 
  filter(abs(delay) == max(abs(delay))) %>%
  distinct(start_date, trip_id, .keep_all = TRUE) %>% 
  ungroup()
...
updates_segment %>% 
  summarise(n_distinct(start_date, trip_id))
```

```{r}
top_first_last_stops %>% 
  ggplot(aes(row, cumsum)) +
  geom_line() +
  facet_wrap(vars(direction_id)) +
  theme_minimal()
```


```{r}
# ggplot(updates_segment, aes(delay)) +
#   geom_histogram()
# 
...
ggplot(updates_stop_times, aes(delay, to_stop_time)) +
  geom_point()
...
ggplot(time_delay, aes(trip_time, delay)) +
  geom_point()

```

```{r}
test <- updates_stop_times %>% 
  filter(start_date == "2022-03-28", trip_id == 642844)

test1 <- updates_trip_time %>% 
  filter(start_date == "2022-03-29", trip_id == 642707)
```
