...
```{r}
# Aggregate buses to 5 minutes
## Ceiling date to 5 minute intervals. Get time
trip_times_5min <- trip_times %>% 
  mutate(arrival_time = ceiling_date(arrival_time, "5 mins"),time = hms::as_hms(arrival_time),
         rush = case_when(
           between(as_hms(arrival_time), as_hms("06:00:00"), as_hms("10:00:00")) ~ "morning",
           between(as_hms(arrival_time), as_hms("15:00:00"), as_hms("19:00:00")) ~ "evening",
           TRUE ~ "neither"),
...
## Average trip time every 5 minutes
trip_times_5min_agg <- trip_times_5min %>% 
  group_by(arrival_time, rush, direction) %>% 
  summarise(number_buses = n(),
            bus_time = round(mean(trip_time))) %>% 
  ungroup()

# Calculate travel time across all links per time aggregate
all_links_time <- link_stats %>% 
  group_by(logtime, direction) %>% 
  summarise(links_time = sum(tt)) %>% 
  ungroup()

# Combine
travel_times <- trip_times_5min_agg %>%
  inner_join(all_links_time, by = c("arrival_time" = "logtime", "direction")) %>% 
  rename(datetime = arrival_time) %>% 
  mutate(day = date(datetime),
         time = as_hms(datetime),
         travel_diff = bus_time - links_time) %>% 
  relocate(day, time, .after = datetime)
