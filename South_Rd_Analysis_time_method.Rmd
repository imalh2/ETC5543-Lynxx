...
```{r}
stops_segment <- clean_names(routes_trips_stops_raw) %>% 
...
updates <- updates_raw %>%
  ...
  # Or slightly incorrect prediction (1 wrong row only in trip)
  filter(arrival_time > lag(arrival_time, default = ymd("0000-01-01", tz = "UTC")),
         arrival_time < lead(arrival_time, default = ymd("9999-01-01", tz = "UTC"))) %>%
  # Remove trips where arrival_time goes back for upcoming stops
  filter(all(arrival_time > lag(arrival_time, default = ymd("0000-01-01", tz = "UTC")))) %>%
  # Remove trips that have stop(s) with delay over 2500
...
updates_in_segment <- updates %>% 
  filter(stop_id %in% stops_segment)

test <- updates %>% 
  filter(start_date == "2022-03-10", trip_id == 643512)
...
updates %>% 
  count(start_date, trip_id, stop_id, sort = TRUE)

ggplot(updates, aes(to_stop_time)) +
  geom_histogram()
...
ggplot(updates,aes(to_stop_time, delay)) +
  geom_point()
```
