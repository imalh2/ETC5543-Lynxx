...
Bus travel times are calculated as the time taken between the first and last stops of a segment, removing the possibility that we are measuring how accurately the schedule predicts and/or buffers for congestion.
...
-   the rate of decline of bus resilience to congestion by examining prior periods of the same road segment
...
Figures \@ref(fig:marion-nb-calendar) and \@ref(fig:marion-sb-calendar) show that both travel types follow a similar travel time pattern. However, the difference in travel times between the types is much wider than that exhibited by South Road per figures \@ref(fig:nb-calendar) and \@ref(fig:sb-calendar).
...
![](here(images/all_large_delays.png))
...
![](here(images/suddent_large_delay.png))
...
![](here(images/arrive_later_stamp_earlier.png))

<br>

4.  A stop's arrival time is earlier than previous stops and the timestamp is older

![](here(images/arrive_earlier_stamp_older.png))
...
![](here(images/same_timestamp_still_error.png))
