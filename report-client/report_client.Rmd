...
The analyses contained in this report seeks to provide the South Australia Department for Infrastructure and Transport (DIT) a measure of bus network resilience to road congestion by examining the relationship between the travel times of both motor vehicles and bus trips, especially during morning and evening rush hours. The core objective of the analysis is to determine for a specified road segment: the extent to which the variation of motor vehicle travel times from normal relative to the same time frame, is matched with such a variation in bus travel times, where a lower correlation indicates a more resilient bus network to increasing road congestion. Bus travel times are calculated using trip updates data obtained from the open source General Transit Feed Specification Realtime (GTFSR) by calculating the time taken between the first and last stops of the trips on the segment, while the motor vehicle travel times are calculated using links metrics available through DIT's Addinsight data lake.

Upon completing the descriptive analysis of the travel times relationship, the travel times were standardized relative to the time of day to better examine the response of the bus travel times to variations in the motor vehicle travel times. The analysis was performed on South Road for the period of March 2022, for both the northbound direction towards the city, and the southbound direction away from the city. The analysis shows that the evening southbound correlation is strong between the standardized travel times, however the variations are relatively small, therefore it is difficult to infer the bus transportation robustness to congestion as larger variation magnitudes would have to be observed and examined. It also finds that the morning standardized travel times towards the city are more varied and the correlation is lower, implying the bus transportation is relatively more robust to congestion than evening bus travel from the city.

The analysis was also performed on Marion Road to compare with South Road. It shows that while the correlation between the standardized travel times of both types is low during the rush hours, a conclusion about the resilience of the bus network to road congestion cannot be made, since the absolute travel times of motor vehicles do not vary greatly and remain relatively stable across the day and across the period. Rather, this is an indication of the greater separation between both travel types as the correlations between the absolute travel times are also low, signalling that road congestion is not a big factor on bus travel times. This is in contrast to South Road, where the correlation figures between the absolute travel times are high, signalling that road congestion does in fact play a large role in bus travel times, hence the standardized travel times correlation figures are more directly related to the bus network resilience to road congestion. The differing results between both roads may be a consequence of South Road being more widely used by commuters to travel to and from the city than Marion Road.

In addition, the analyses attempted to validate the DIT Addinsight data, specifically the links five minute statistics. The data required some cleaning as invalid figures were found solely relating to the travel time metric. Comparing the travel times between the motor vehicles through the links data, and the buses through the GTFSR data, found that the two datasets exhibit similar travel time patterns in general, indicating the data set is valid as we would not expect to see great variations in the patterns relative to the travel types contained in Addinsights and GTFSR. Therefore, while the source of the errors would have to be identified and fixed (possibly arising during the database entry phase), a great degree of confidence can be assigned to the Addinsight data.
...
knitr::include_graphics(here("images/SouthRd_Map.png"))
```
...
The method to identify the links, as with the analysis in general, attempts to ensure maximum automation and minimal manual input. Each direction of travel will be processed separately using the same method. The northbound direction will be used here as an illustration.
...
```{r, fig.cap="Marion Rd on map. Source: Google Maps"}
knitr::include_graphics(here("images/MarionRd_Map.png"))
```
...
```{r}
# Stops
segment_routes_stops <- clean_names(marionrd_routes_stops_raw)
...
rm(marionrd_routes_stops_raw)
rm(full_stops_raw)

# Updates and Trip Times
updates <- marionrd_updates_raw %>%
  rename(
    trip_id = id,
    vehicle_id = label) %>%
  mutate(
...
rm(marionrd_updates_raw)

trip_times_agg_marionrd <- trip_times %>% 
  mutate(arrival_time = ceiling_date(arrival_time, "5 minutes")) %>% 
  group_by(arrival_time, direction) %>% 
  summarise(
    number_buses = n(),
...
# Links
links_segment <- marionrd_links_raw %>%
  rename_with(tolower) %>%
  mutate(
    latitude = round(latitude, 5),
    longitude = round(longitude, 5),
...
# Link Stats
link_stats <- marionrd_link_stats_raw %>% 
  select(logtime:avgdensity) %>%
  mutate(logtime = ymd_hms(str_remove(logtime, " \\+10:30"), tz = "Australia/Adelaide")) %>%
  filter(
    !as_date(logtime) %in% holidays,
...
# Analysis
travel_times_marionrd <- trip_times_agg_marionrd %>%
  inner_join(links_times) %>% 
  mutate(travel_diff = bus_time - links_time)

travel_times_marionrd_piv <- travel_times_marionrd %>% 
  pivot_longer(cols = bus_time:links_time, names_to = "type", values_to = "travel_time") %>% 
  mutate(type = recode(type, bus_time = "Bus", links_time = "Vehicle"))
```
...
```{r marion-bus-vehicles-time, fig.cap="Travel time distributions are similar across rush hours and directions"}
travel_times_marionrd_piv %>% 
  filter(rush != "Neither") %>% 
  ggplot(aes(travel_time/60, fill = type)) +
  geom_density(alpha = 0.4) +
  facet_grid(vars(direction), vars(rush)) +
  scale_x_continuous(labels = label_comma(), limits = c(min(travel_times_marionrd_piv$travel_time/60), 25)) +
  labs(x = "Travel Time (Minutes)", fill = "Type", title = "Bus Travel Time vs Vehicle Travel Time") +
  theme_bw()
```
...
From figure \@ref(fig:marion-bus-vehicles-time) we learn the travel times distributions are similar across rush hours and directions. This is in comparison to the South Road distributions in figure \@ref(fig:bus-vehicles-time) where the travel times of both types are varied between rush hours and directions.

<br>

```{r marion-nb-calendar, fig.height=8, fig.width=9, fig.cap="Northbound bus travel times are slower than vehicle travel times. Both exhibit similar patterns"}
# Direction 1 calendar plot
travel_times_piv_dir1 <- travel_times_marionrd_piv %>% 
  filter(direction == directions[1])

travel_times_piv_dir1 %>% 
  filter(time > as_hms("00:00:00")) %>% 
  ggplot(aes(time, travel_time, color = type)) +
  geom_line() +
  facet_wrap(vars(day), scales = "free_y") +
  scale_x_time(labels = label_time("%H:%M")) +
...
```{r marion-sb-calendar, fig.height=8, fig.width=9, fig.cap="Southbound bus travel times are slower than vehicle travel times. Both exhibit similar patterns"}
# Direction 2 calendar plot
travel_times_piv_dir2 <- travel_times_marionrd_piv %>% 
  filter(direction == directions[2])

travel_times_piv_dir2 %>% 
  filter(time > as_hms("01:00:00")) %>% 
  ggplot(aes(time, travel_time, color = type)) +
  geom_line() +
  facet_wrap(vars(day), scales = "free_y") +
  scale_x_time(labels = label_time("%H:%M")) +
...
Figures \@ref(fig:marion-nb-calendar) and \@ref(fig:marion-sb-calendar) show that both travel types follow a similar travel time pattern.

<br>

```{r marion-avg-pattern, fig.cap="Average travel time patterns by vehicle and direction. Peak times are highlighted"}
# Average total links time in each 30 minute bin
links_30min <- link_stats %>%
  group_by(logtime, direction) %>%
  summarise(links_time = sum(tt)) %>%
...
# Average bus trip times in each 30 minute bin
trip_times_30min_marionrd <- trip_times %>% 
  mutate(arrival_time = ceiling_date(arrival_time, "30 minutes"),
         time = as_hms(arrival_time)) %>% 
  group_by(time, direction) %>% 
  summarise(bus_time = round(mean(trip_time))) %>%
...
links_trips_30min <- links_30min %>% 
  inner_join(trip_times_30min_marionrd) %>% 
  pivot_longer(cols = links_time:bus_time, names_to = "type", values_to = "value") %>% 
  mutate(type = recode(type, bus_time = "Bus", links_time = "Vehicle"))
...
Figure \@ref(fig:marion-avg-pattern) shows that both travel types display a small uptick in travel time towards the city during the morning rush hour. In the southbound direction vehicles have a slightly larger travel time during the evening, while buses exhibit an increase in travel time during both rush hours. Figure \@ref(fig:avg-pattern) pertaining to South Road shows that the southbound direction in particular has a large increase in both travel times during the evening rush hour.

<br>

```{r marion-times-scatter, fig.cap="A positive relationship exists between both travel times during the morning towards the city"}
# Scatter plots
travel_times_marionrd %>% 
  filter(rush != "Neither") %>% 
  ggplot(aes(bus_time/60, links_time/60)) +
  geom_point(alpha = 0.3) +
  scale_x_continuous(labels = label_comma()) +
...
Figure \@ref(fig:marion-times-scatter) shows that a positive relationship exists between both travel types during the morning rush hour towards the city. While a relationship does not seem present during the evening rush hour away from the city.

<br>

```{r marion-general-corr}
travel_times_marionrd %>% 
  filter(rush != "Neither") %>% 
  group_by(rush, direction) %>% 
  summarise(Correlation = round(cor(bus_time, links_time), 2)) %>% 
...
Table \@ref(tab:marion-general-corr) displays the correlation figures between the travel times. Only the travel times during the morning towards the city show a relatively strong correlation.
...
standardiser <- function(basis = hour) {
  ...
  trip_times_marionrd_std <- trip_times_agg_marionrd %>%
    group_by({{basis}}, direction) %>% 
    mutate(
      period_buses_no = n(),
      bus_time_std = as.vector(scale(bus_time))) %>% 
  ...
  # Combine
  travel_times_marionrd_std <<- trip_times_marionrd_std %>%
    inner_join(links_times_std) %>%
    mutate(std_travel_diff = bus_time_std - links_time_std)
  
  travel_times_marionrd_std_piv <<- travel_times_marionrd_std %>% 
    pivot_longer(cols = bus_time_std:links_time_std, names_to = "type", values_to = "std_travel_time") %>% 
    mutate(type = recode(type, bus_time_std = "Bus", links_time_std = "Vehicle"))
}
...
```{r marion-nb-std-calendar, fig.height=8, fig.width=9, fig.cap="Northbound morning travel time variations seem moderately correlated"}
# Morning Northbound
travel_times_std_NB_morn <- travel_times_marionrd_std %>% 
  filter(direction == "NB",
         rush == "Morning")

travel_times_std_piv_NB_morn <- travel_times_marionrd_std_piv %>% 
  filter(direction == "NB",
         rush == "Morning")
...
```{r marion-sb-std-calendar, fig.height=8, fig.width=9, fig.cap="Southbound evening travel time variations are not similar"}
# Evening Southbound
travel_times_std_SB_eve <- travel_times_marionrd_std %>% 
  filter(direction == "SB",
         rush == "Evening")

travel_times_std_piv_SB_eve <- travel_times_marionrd_std_piv %>% 
  filter(direction == "SB",
         rush == "Evening")
...
Figures \@ref(fig:marion-nb-std-calendar) and \@ref(fig:marion-sb-std-calendar) show that the variations in vehicle travel times are not closely matched by variations in bus travel times. This is in contrast to South Road where figures \@ref(fig:nb-std-calendar) and \@ref(fig:sb-std-calendar) show a close relationship between the variations in travel times of both types.

<br>

```{r marion-std-time-dist, fig.cap="The extent of the variability in travel times is similar across the rush hours and travel directions"}
# Travel time distributions
travel_times_marionrd_std_piv %>% 
  filter(rush != "Neither") %>% 
  ggplot(aes(std_travel_time, fill = type)) +
  geom_density(alpha = 0.4) +
  facet_grid(vars(direction), vars(rush)) +
...
Figure \@ref(fig:marion-std-time-dist) shows that travel times within the same time period generally possess the same level of variation across the rush hour periods and travel directions.

<br>

```{r marion-std-scatter, fig.cap="No strong relationships are present"}
travel_times_marionrd_std %>% 
  filter(rush != "Neither") %>% 
  ggplot(aes(links_time_std, bus_time_std)) +
  geom_point(alpha = 0.3) +
  facet_grid(vars(direction), vars(rush)) +
...
Figure \@ref(fig:marion-std-scatter) and table \@ref(tab:marion-std-corr) show that a slight relationship exists between the standardized travel times during the morning rush hour towards the city, and no relationship exists otherwise. South Road, on the other hand, displays a positive relationship between the standardized travel times during the morning towards the city and during the evening away from the city, as shown in \@ref(fig:std-scatter) and \@ref(tab:std-corr).
...
-   While the correlation between the standardized travel times of both types is low during the rush hours, a conclusion about the resilience of the bus network to road congestion cannot be made, since the absolute travel times of motor vehicles do not vary greatly and remain relatively stable across the day and across the period.
