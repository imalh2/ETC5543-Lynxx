...
# Missing link between Daws Road and Celtic Avenue. Locate a link between the two
link_daws_celt_up <-
  links_south_rd_dir %>%
  filter(str_detect(tolower(name), "daws"),
...
# Missing link between Cross Road and Corunna Avenue. Locate a link between the two
link_cross_cor_up <-
  links_south_rd_dir %>%
  filter(str_detect(tolower(name), "cross") |
         str_detect(tolower(name), "corunna"),
         going_up == TRUE,
         !linkid %in% unique(links_map_up$linkid))

# Missing link between Richmond Road and Barwell Avenue. Locate a link between the two
link_rich_bar_up <-
  links_south_rd_dir %>%
  filter(str_detect(tolower(name), "richmond"),
         str_detect(tolower(name), "everard"),
         going_up == TRUE)

map_up %>%
  addPolylines(data = link_daws_celt_up, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1) %>%
  addPolylines(data = link_cross_cor_up, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1) %>% 
  addPolylines(data = link_rich_bar_up, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1)

# Combine missing links with map links up DF and plot
links_map_up <- links_map_up %>% 
  rbind(link_daws_celt_up, link_cross_cor_up, link_rich_bar_up)

leaflet(links_map_up) %>% 
  addTiles() %>% 
...
# Missing link between Raglan Avenue and Corunna Avenue. Locate a link between the two
link_cor_rag_down <-
  links_south_rd_dir %>%
  filter(str_detect(tolower(name), "corunna") |
         str_detect(tolower(name), "raglan"),
         going_up == FALSE) %>% 
  filter(linkid == 698)
...
link_alb_edw_down <-
  links_south_rd_dir %>%
  filter(str_detect(tolower(name), "albert"),
         str_detect(tolower(name), "edward"),
         going_up == FALSE)

# Missing link between Richmond Street and below. Locate a link between the two
link_rich_down <-
  links_south_rd_dir %>%
  filter(str_detect(tolower(name), "richmond"),
         going_up == FALSE) %>% 
...
# Missing link between Everard Avenue and Anzac Highway. Locate a link between the two
link_anz_ever_down <-
  links_south_rd_dir %>%
...
map_down %>% 
  addPolylines(data = link_cor_rag_down, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1) %>% 
  addPolylines(data = link_alb_edw_down, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1) %>% 
  addPolylines(data = link_rich_down, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1) %>% 
  addPolylines(data = link_anz_ever_down, lng = ~longitude, lat = ~latitude, color = "yellow", label = ~linkid, opacity = 1)

# Combine missing links with map links down DF and plot
links_map_down <- links_map_down %>%
  # Remove link 1963 since it is now covered by 624
  filter(linkid != 1963) %>% 
  rbind(link_cor_rag_down, link_alb_edw_down, link_rich_down, link_anz_ever_down)

leaflet(links_map_down) %>% 
  addTiles() %>% 
  addPolylines(lng = ~longitude, lat = ~latitude)

# Map with both directions
leaflet() %>% 
  addTiles() %>% 
  addPolylines(data = links_map_up, lng = ~longitude, lat = ~latitude, color = "green") %>%
  addPolylines(data = links_map_down, lng = ~longitude, lat = ~latitude, color = "red")
```
